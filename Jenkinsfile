pipeline {
    ...
    environment {
        ...
        DOCKERHUB_USERNAME = 'noni64'  // Set this directly to your Docker Hub username
    ...
    stages {
        stage('Git Checkout') {
            steps {
               git branch: 'main', url: 'https://github.com/Noni-arch/SonarQube-Project-Kastro.git'
            }
        }
        ...
        stage('Sonar Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=Noni -Dsonar.projectKey=Nonikey -Dsonar.java.binaries=target"
                }
            }
        }
